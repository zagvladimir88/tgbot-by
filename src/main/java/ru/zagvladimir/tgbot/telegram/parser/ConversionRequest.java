...
public record ConversionRequest(BigDecimal amount, String from, String to) {

    private static final Pattern PATTERN = Pattern.compile(
            "^\\s*(\\d+(?:[.,]\\d+)?)\\s*([a-zA-Zа-яА-Я]{3})\\s*(?:(?:в|to|->)\\s*)?([a-zA-Zа-яА-Я]{3})?\\s*$");

    public static Optional<ConversionRequest> parse(String input, String defaultTarget) {
